...
export default function RegisterScreen() {
  ...
  // Validasi sederhana: Tombol aktif jika field wajib terisi dan password cocok
  const isFormValid = name && phone && password && (password === confirmPassword);

  const handleRegister = () => {
    console.log('Mendaftar dengan:', { name, storeName, phone, password });
  };
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
        <View className="flex-1 px-6" style={{ paddingTop: insets.top + 20, paddingBottom: 20 }}>
          ...
          <View className="space-y-4">
            ...
            {/* Input Nama Toko (Opsional) */}
            <View>
              <Text className="text-sm font-bold text-gray-700 mb-1.5 ml-1">Nama Toko (Opsional)</Text>
              <View className="flex-row items-center bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                <Ionicons name="storefront-outline" size={20} color="gray" />
                <TextInput
                  placeholder="Nama toko / usaha kamu"
                  className="flex-1 ml-3 text-base text-gray-800"
                  value={storeName}
          ...
          <View className="mt-4">
            <TouchableOpacity 
              className={`w-full py-4 rounded-xl items-center shadow-sm ${isFormValid ? 'bg-red-500' : 'bg-red-300'}`}
